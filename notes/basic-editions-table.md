...
You can publish this as `HTML` and save it as a Store Entry.

Use Venus.

The Store Key is `basic-editions-table.html`.

Only copy the `innerHTML` for the `TABLE`.

There's also a `JSON` version and a `CSV` version. Keep these
updated (sync with this table).
