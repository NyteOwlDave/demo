...
// VIEW (vip) : lissajous-viewer.js
// EDIT (sce) : heds-up-editor.js
...
function init_viewer() {
	...
	vip.stash = function() {
		const s = vip.textContent
		vip.saved = ( s || vip.saved || "" );
   		console.log( `Stashed "vip" content` );
	};
	vip.resume = function() {
		const s = ( vip.saved || "" );
		const t = ( vip.textContent );
		vip.textContent = ( s || t );
	};
...
<script id="view_nav_links.js">
function view_nav_links() {
   	const vip = init_viewer();
	vip.stash();
	vip.innerText = nav_links;
}
</script>

<script id="view_members.js">
function view_catalog( o, sort ) {
   	const vip = init_viewer();
	vip.stash();
	o = ( o || localStorage );
	if ( sort ) {
...
+ Venus
@ http://dave-omega/app/jarvis/toolkit/ncs/venus/
